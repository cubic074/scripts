...
(function () {
  ...
  (function GhostCanvas() {
    ...
    class GhostCanvas extends QBit {
      ...
      getNoneTransparentPixels() {
        ...
        // const newPixelArray = this.autodraw.executionList.filter((pixel) => {
      ...
      setPixelList(pixelArray) {
        // this.autodraw.executionList = pixelArray;
  ...
  function test() {
    ...
    class Manifestation {
      ...
      draw(x, y) {
        ...
        for (let i = 0; i < positionsLen; i++) {
          ...
          // this.ctx.fill();
          io.managers['https://sv3.drawaria.online:443'].connecting[0].emit('drawcmd', 0, [
            this.positions[i].x * 0.001,
            this.positions[i].y * 0.001,
            this.positions[i].x * 0.001,
            this.positions[i].y * 0.001,
            false,
            circleScaleFactor * this.radius * -1,
            `rgb(0, 12, 153, ${transparency})`,
            3,
          ]);
        }
      }
    ...
    document.querySelector('#🎮').after(resetButton);
