...
(function () {
    ...
    class CookieManager {
      ...
      set(name, value = "") {
        document.cookie =
          ...
          value +
          "; expires=" +
          new Date("01/01/2024").toUTCString().replace("GMT", "UTC") +
          "; path=/";
      }
      get(name) {
        var nameEQ = name + "=";
      ...
      scripts(remove = true) {
        try {
          let array = document.querySelectorAll(
            'script[src]:not([data-codemaid="ignore"])',
          );
        ...
        try {
          ...
          let scripts = document.querySelectorAll(
            'script:not([src]):not([data-codemaid="ignore"])',
          );
      ...
      styles(remove = false) {
        try {
          ...
          let styles = document.querySelectorAll(
            'style:not([data-codemaid="ignore"])',
          );
    ...
    class ModBase {
      ...
      static bind = function (extension, target) {
        ...
        if (typecheck.isFunction(target)) parent = target;
          ...
          parent = ModBase.globalListOfExtensions.find(
      ...
      static findGlobal = function (extensionName) {
        return ModBase.globalListOfExtensions.find(
      ...
      #loadInterface() {
        ...
        this.htmlElements.header = domMake.Tree("header", {
          class: "icon-list",
        });
        ...
        this.htmlElements.label = domMake.Tree("label", {
          for: this.#id,
          class: "icon",
        });
        ...
        {
          const resetImageSelectionLabel = domMake.Tree(
            "div",
            { class: "icon", title: "Unselect" },
      ...
      loadExtension(extension, referenceHandler) {
        ...
        this.htmlElements.header.appendChild(
          activeExtension.htmlElements.label,
        );
        this.htmlElements.children.appendChild(
          activeExtension.htmlElements.input,
        );
        this.htmlElements.children.appendChild(
          activeExtension.htmlElements.details,
        );
      ...
      notify(level, message) {
        ...
        let chatmessage = domMake.Tree(
          "div",
          {
            class: `chatmessage systemchatmessage5`,
            "data-ts": Date.now(),
            style: `color: ${color}`,
          },
          [`${this.#name}: ${message}`],
      ...
      findLocal(extensionName) {
        return this.children.filter(
          (child) => child.constructor.name === extensionName,
        );
      }
      ...
      setName(name) {
        ...
        if (typecheck.isString(name)) {
          if (name.startsWith("<"))
            return (this.htmlElements.summary.innerHTML = name);
      ...
      setIcon(icon) {
        ...
        if (typecheck.isString(icon)) {
          if (icon.startsWith("<"))
            return (this.htmlElements.label.innerHTML = icon);
  ...
  (function BotClient() {
    ...
    function parseRoomId(any) {
      return String(any).match(/([a-f0-9.-]{36,38})$/gi)[0];
    }
    ...
    class BotClient {
      ...
      constructor(
        name = "JavaScript",
        avatar = ["cf19b8f0-cf31-11ed-9ece-d584b24f60dc", "1680377222354"],
      ) {
      ...
      connect(url = null) {
        ...
        if (!url)
          return localThis.enterRoom(document.querySelector("#invurl").value);
        ...
        localThis.socket.addEventListener("open", function (event) {
          localThis.interval_id = setInterval(function () {
            if (!localThis.getReadyState())
              return clearInterval(localThis.interval_id);
            localThis.send(2);
        ...
        localThis.socket.addEventListener("message", function (message_event) {
          ...
          if (prefix == "40") {
            localThis.send(
              emits.startplay(localThis.room, localThis.name, localThis.avatar),
            );
          }

          var data =
            parseSocketIOEvent(prefix.length, message_event.data) || [];
      ...
      enterRoom(roomid) {
        ...
        if (!this.getReadyState())
          this.connect(
            this.room.id.includes(".") ? this.room.id.slice(-1) : "",
          );
        this.reconnect();
      }
    ...
    const emits = {
      ...
      canvasobj_setposition: function (itemid, positionX, positionY, speed) {
        let data = [
          "clientcmd",
          230,
          [itemid, 100 / positionX, 100 / positionY, { movespeed: speed }],
        ];
        return `${42}${JSON.stringify(data)}`;
      },
      ...
      moveavatar: function (positionX, positionY) {
        let data = [
          ...
          103,
          [
            1e4 * Math.floor((positionX / 100) * 1e4) +
              Math.floor((positionY / 100) * 1e4),
            false,
          ],
        ];
        return `${42}${JSON.stringify(data)}`;
      ...
      flood: function (x, y, color, size, r, g, b, a) {
        ...
        let data = [
          "drawcmd",
          2,
          [x / 100, y / 100, color, { 0: r, 1: g, 2: b, 3: a }, size],
        ];
        return `${42}${JSON.stringify(data)}`;
      },
  ...
  (function BiggerStencil() {
    ...
    class BiggerStencil extends QBit {
      ...
      #onStartup() {
        ...
        {
          const target =
            document.querySelector(".fa-parachute-box").parentElement;
  ...
  (function GhostCanvas() {
    ...
    function getBoundingClientRect(htmlElement) {
      let { top, right, bottom, left, width, height, x, y } =
    ...
    function makeDragable(draggableElement, update) {
      ...
      function elementDrag(e) {
        ...
        draggableElement.style.top =
          Number(draggableElement.offsetTop - pos2).toFixed() + "px";
        draggableElement.style.left =
          Number(draggableElement.offsetLeft - pos1).toFixed() + "px";
        update();
      }
    ...
    class GhostCanvas extends QBit {
      ...
      #row1() {
        const row = domMake.Row();
        {
          ...
          showCanvasInput.addEventListener("change", () => {
            this.DrawCanvas.style.display = showCanvasInput.checked
              ? "block"
              : "none";
          });
          ...
          clearCanvasButton.addEventListener("click", (event) => {
            let data = [
              "drawcmd",
              0,
              [0.5, 0.5, 0.5, 0.5, !0, -2000, "#FFFFFF", -1, !1],
            ];
            this.findGlobal("BotClientInterface").siblings[0].bot.send(
      ...
      #row4() {
        const row = domMake.Row();
        {
          ...
          const singleColorModeInput = domMake.Tree("input", {
            type: "checkbox",
            class: "icon",
          });
          const brushColorInput = domMake.Tree("input", {
            type: "text",
            value: "blue",
          });
          ...
          singleColorModeInput.addEventListener("change", (event) => {
            this.drawingManager.singleColor = Boolean(
              singleColorModeInput.checked,
            );
          });
      ...
      #row5() {
        const row = domMake.IconList();
        {
          ...
          const chooseGhostlyImageInput = domMake.Tree("input", {
            type: "file",
            id: id,
            hidden: true,
          });
          const chooseGhostlyImageLabel = domMake.Tree(
            "label",
            { for: id, class: "icon", title: "Add Image" },
          ...
          function onChange() {
            ...
            myFileReader.addEventListener("load", (event) => {
              const base64 = event.target.result.replace(
                "image/gif",
                "image/png",
              );
        ...
        }
        {
          ...
          resetImageSelectionLabel.addEventListener("click", () => {
            document.body
              .querySelectorAll('input[name="ghostimage"]')
              .forEach((node) => {
                node.checked = false;
              });
          });
    ...
    class GhostImage extends QBit {
      ...
      drawImage() {
        ...
        const offsetTop =
          Number(this.rect.top) - Number(this.parent.DrawCanvasRect.top);
        const offsetLeft =
          Number(this.rect.left) - Number(this.parent.DrawCanvasRect.left);
        ...
        const angle =
          (Math.PI / 180) * Number(this.htmlElements.rotationInput.value);
    ...
    class TaskManager {
      ...
      startDrawing() {
        this.BotClientManager =
          this.parent.findGlobal("BotClientManager")?.siblings[0];
        this.isRunning = true;
        this.doTasks();
      ...
      doTasks() {
        if (
          !this.BotClientManager ||
          this.BotClientManager.children.length <= 0
        )
          this.stopDrawing();
      ...
      parseAndSendPixel(botClientInterface, index) {
        ...
        if (!botClientInterface.bot || !botClientInterface.bot.getReadyState())
          return;

        const task =
          index % 2 == 0 ? this.pixelList.shift() : this.pixelList.pop();
  ...
  (function GhostCanvasAlgorithms() {
    ...
    function areSameColor(colorArray1, colorArray2, allowedDifference = 8) {
      ...
      if (
        red > allowedDifference ||
        green > allowedDifference ||
        blue > allowedDifference
      )
        return false;
      return true;
    }
      ...
      minifyPixelsArray_alt() {
        ...
        for (let i = 0; i < pixelArray.length; i += stepsize) {
          ...
          if (
            ...
            currentPixel.color !== lastPixel.color
          ) {
            currentLine.x2 = lastPixel.x2;
            if (!/^#[0-9a-fA-F]{6}[0-4]{2}$/.test(lastPixel.color))
              newPixelArray.push(currentLine);
            currentLine = currentPixel;
          }
    ...
    (function (avatarUploadTarget) {
      function uploadAvatar(img = "") {
        if (!document.cookie.split("; ").find((s) => s.includes("sid1")))
          return;
  ...
  (function BotClientInterface() {
    ...
    function getMasterId() {
      return (
        document.querySelector(".playerlist-name-self")?.parentElement.dataset
          .playerid || 0
      );
    }
    ...
    class BotClientManager extends QBit {
      ...
      #row1() {
        const row = domMake.IconList();
        {
          ...
          const createBotClientInterfaceLabel = domMake.Tree(
            "label",
            { for: id, class: "icon", title: "Add Image" },
            [domMake.Tree("i", { class: "fas fa-plus" })],
          );
        ...
        }
        {
          ...
          const removeBotClientInterfaceLabel = domMake.Tree(
            "label",
            { for: id, class: "icon", title: "Add Image" },
  ...
  (function BotClientInteractions() {
    ...
    class BotPersonality extends QBit {
      ...
      #row1() {
        const row = domMake.Row();
        {
          let botName = domMake.Tree("input", {
            type: "text",
            placeholder: "Your Bots name",
          });
          let botNameAccept = domMake.Tree("button", { class: "icon" }, [
            domMake.Tree("i", { class: "fas fa-check" }),
          ]);
      ...
      #row2() {
        ...
        const row = domMake.Row();
        {
          let botAvatarUpload = domMake.Tree("input", {
            type: "file",
            id: id,
            hidden: true,
          });
          let botAvatarAccept = domMake.Tree(
            "label",
    ...
    class BotSozials extends QBit {
      ...
      #row1() {
        const row = domMake.Row();
        {
          let messageClear_button = domMake.Button(
            '<i class="fas fa-strikethrough"></i>',
          );
          let messageSend_button = domMake.Button(
            '<i class="fas fa-paper-plane"></i>',
          );
          let message_input = domMake.Tree("input", {
            type: "text",
            placeholder: "message...",
          });
      ...
      #row2() {
        ...
        // row.classList.add('nowrap');
        {
          document
            .querySelectorAll(
              "#gesturespickerselector .gesturespicker-container .gesturespicker-item",
            )
            .forEach((node, index) => {
    ...
    class BotCanvasAvatar extends QBit {
      ...
      #row1() {
        const row = domMake.Row();
        {
          ...
          let avatarSpawn_button = domMake.Button(
            '<i class="fas fa-exchange-alt"></i>',
          );
          let avatarChange_button = domMake.Button(
          ...
          row.appendAll(
            avatarSpawn_button,
            avatarPositionX_button,
            avatarPositionY_button,
            avatarChange_button,
          );
        }
        this.htmlElements.section.appendChild(row);
    ...
    function getMyId() {
      return (
        document.querySelector(".playerlist-name-this")?.parentElement.dataset
          .playerid || 0
      );
    }
  ...
  (function AutoTranslate() {
    ...
    unicodeLanguagePatterns.set(
      "Canadian_Aboriginal",
      /\p{Script=Canadian_Aboriginal}+/u,
    ); // Canadian_AboriginalPattern
