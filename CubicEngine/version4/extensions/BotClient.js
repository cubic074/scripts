...
// @name         BotClient
// @version      3.8
// @description  Class to create multiple BotPlayers
...
(function BotClient() {
  ...
  const emits = {
    ...
    canvasobj_remove: function (itemid) {
      const data = ['clientcmd', 232, [itemid]];
      return `${42}${JSON.stringify(data)}`;
    },
    ...
    startplay: function (room, name, avatar) {
      const data = `${420}${JSON.stringify(['startplay', name, room.type, 'en', room.id, null, [null, 'https://drawaria.online/', 1000, 1000, [null, avatar[0], avatar[1]], null]])}`;
      return data;
    },
    ...
    erase: function (playerid, lastx, lasty, x, y, isactive, size, color) {
      const data = ['drawcmd', 1, [lastx / 100, lasty / 100, x / 100, y / 100, isactive, -size, color, playerid]];
      return `${42}${JSON.stringify(data)}`;
    },
    flood: function (x, y, color, size, r, g, b, a) {
      // 42["drawcmd",2,[x, y,color,{"0":r,"1":g,"2":b,"3":a},size]]
      const data = ['drawcmd', 2, [x / 100, y / 100, color, { 0: r, 1: g, 2: b, 3: a }, size]];
      return `${42}${JSON.stringify(data)}`;
    },
