...
(function (identifier) {
  ...
  class BotClient {
    static dummy = Addon.register('BotClient', this);
    ...
    connect(url) {
      ...
      localThis.socket.addEventListener('message', function (message_event) {
        ...
        if (data && data.length > 1) {
          ...
          var observer = localThis.customObservers[event];
  ...
  globalThis['socketio'] = { events, emits };
})('CubeEngine');
