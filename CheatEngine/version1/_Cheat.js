// ==UserScript==
// @name         Cheat -
// @namespace    CheatEngine
// @version      1.0.0
// @description
// @author       ≺ᴄᴜʙᴇ³≻
// @match        https://*.drawaria.online/*
// @icon64       data:image/png;base64,
// @grant        none
// @license      GNU GPLv3
// @run-at       document-idle
// ==/UserScript==

(function (scope) {
  let helper = scope['CodeMaid'];
  let Engine = scope['🎮'];
  let Cheat = scope['📦'];

  class myCheat extends Cheat {
    static dummy = Cheat.bind(this, 'main'); // <- set which module should load this as child

    constructor(parentClassReference) {
      super(parentClassReference); // <- this.initialize()
      // define initialize function to prevent the creation of html base

      this.setIcon(helper.createDOM.FA('<i class="fa-duotone fa-paintbrush"></i>'));

      this.init();
    }

    init() {
      this.#row1();
      this.enable();
    }

    enable() {
      this.active = true;

      this.enableButton.classList.add('active');
      this.enableButton.textContent = 'Active';

      this.log('ok', `${this.name} enabled`);
    }

    disable() {
      this.active = false;

      this.enableButton.classList.remove('active');
      this.enableButton.textContent = 'Inactive';

      this.log('warn', `${this.name} disabled`);
    }

    #row1() {
      let self = this;
      let row = helper.createDOM.Row();
      {
        this.enableButton = helper.createDOM.Button('Enable');

        this.enableButton.addEventListener('click', function (event) {
          self.active ? self.disable() : self.enable();
        });
        row.append(this.enableButton);
      }
      this.body.append(row);
    }
  }
})(globalThis);
